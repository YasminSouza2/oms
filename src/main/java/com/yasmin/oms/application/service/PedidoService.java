...
public class PedidoService {
    ...
    private static final List<String> STATUS_VALIDOS = List.of(
            "AGUARDANDO_PAGAMENTO", "PAGAMENTO_SIMULADO", "PENDENTE", "CONFIRMADO", "ENVIADO", "ENTREGUE", "CANCELADO"
    );
    ...
    public PedidoResponse atualizarStatus(UUID id, PedidoStatusRequest request) {
        ...
        if (!STATUS_VALIDOS.contains(novoStatus)) {
            throw new BusinessException("Status inválido. Use: AGUARDANDO_PAGAMENTO, PAGAMENTO_SIMULADO, PENDENTE, CONFIRMADO, ENVIADO, ENTREGUE ou CANCELADO.");
        }
    ...
    @Transactional
    public PedidoResponse finalizarPagamento(UUID id) {
        Pedido pedido = pedidoRepository.findByIdWithDetails(id)
                .orElseThrow(() -> new ResourceNotFoundException("Pedido", id));

        if (!"AGUARDANDO_PAGAMENTO".equals(pedido.getStatus())) {
            throw new BusinessException("Somente pedidos com status AGUARDANDO_PAGAMENTO podem ter o pagamento finalizado.");
        }

        pedido.setStatus("PAGAMENTO_SIMULADO");
        pedido = pedidoRepository.save(pedido);
        return toResponse(pedido);
    }
